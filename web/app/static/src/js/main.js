...
$(window).ready(function () {
  ...
  $(".close-modal").on("click", function () {
    ...
    setTimeout(function () {
      ...
      $("#event-view, #score-info .score-team").removeClass(function (
        index,
        css
      ) {
...
function admin_panel(action) {
  ...
  if (action == "login") {
    login.css("transform", "translateY(120%)");
    $(
      "#lugobot-view, #lugobot-header, #lugobot-stadium, #event-view, #lugobot-admin-panel"
    ).addClass("debug-mode");
    size_things();
...
function size_things() {
  ...
  if (panel.hasClass("debug-mode")) {
    positionField =
      (screenHeight - (heightNew + 40) - scoreboardHeight - panelHeight) / 2;
  } else {
    positionField =
      (screenHeight - (heightNew + 40) - scoreboardHeight - loginHeight) / 2;
  }
...
// Adicionar Classe no remaining
function class_remaining() {
  $("#timer #remaining").toggleClass("active");
}

// Modals Functions
function modal_game_over() {
  setTimeout(function () {
    $("#event-view #modal-game-over").addClass("zoom-In");
  }, 100);
...
function modal_winner(team) {
  ...
  setTimeout(function () {
    $("#event-view #modal-winner").addClass("zoom-In");
  }, 100);
...
function event_goal(team) {
  var home_bg =
    ...
    getComputedStyle(document.documentElement).getPropertyValue(
      "--team-home-color-primary"
    ) +
    ")";
  var away_bg =
    "rgb(" +
    getComputedStyle(document.documentElement).getPropertyValue(
      "--team-away-color-primary"
    ) +
    ")";
  ...
  setTimeout(function () {
    eventView.addClass("zoom-In");
  }, 1000);

  eventView.addClass("active-modal");
  header.addClass("active-modal");

  $("#event-view, #event-goal, #score-info .score-team").removeClass(function (
    index,
    css
  ) {
    return (css.match(/\bgoal\S+/g) || []).join(" ");
  ...
  setTimeout(function () {
    ...
    setTimeout(function () {
      ...
      $("#event-view, #event-goal, #score-info .score-team").removeClass(
        function (index, css) {
          return (css.match(/\bgoal\S+/g) || []).join(" ");
        }
      );
